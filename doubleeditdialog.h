...
twoedit showme(QWidget *parent) {
    QMessageBox msgBox(parent);
    msgBox.setWindowTitle("更新报修状态");
    msgBox.setMinimumSize(400, 250);

    QLabel *idLabel = new QLabel("报修ID:", &msgBox);
    QLineEdit *idEdit = new QLineEdit(&msgBox);
    idEdit->setPlaceholderText("输入报修编号...");
    idEdit->setFocus();

    QLabel *remarksLabel = new QLabel("新的状态:", &msgBox);
    QLineEdit *remarksEdit = new QLineEdit(&msgBox);
    remarksEdit->setPlaceholderText("输入保修状态...");
    ...
    if (layout) {
        ...
        layout->addWidget(idEdit, 0, 1, 1, 2);

        layout->addWidget(remarksLabel, 1, 0);
        layout->addWidget(remarksEdit, 1, 1, 1, 2);
